...
for (var _line = 0; _line < (visible_options_max + _desc); _line++){
	
	if(_line >= array_length(options)){
		break
	}
	...
	if(_line == 0) && (_desc){
		...
		var _option_to_show = _line - _desc + _scroll_push
		...
		draw_text(x + x_margin, y + y_margin + _line * height_line, _str)
...
if(visible_options_max < array_length(options)) && (hover < array_length(options) - 1){
	...
	draw_sprite(spr_down_arrow, 0,x + width_full * 0.5, y + height_full - 40)	
